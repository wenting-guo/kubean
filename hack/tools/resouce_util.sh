...
#####################################
  ...
  #           value2: BUILD
function resource::init_vars_offline_resource_download(){
  export NEW_TAG=$1
  export STEP_TYPE=$2z
  ...
  export KUBECONFIG_FILE="/root/.kube/offline_resource.config"
  export KIND_NAME="offline-resource"
...
function resource::check_file_integrity(){
  local new_tag=$1
...
#####################################
function resource::download_file_list(){
  local new_tag=$1
  local download_root_path=$2
  local file_url=""
  local file_name=""
  ...
  if [[ $# -gt 3 ]];then
    ...
    wget -q -c -T 1m -P ${download_root_path}/${file_name} "${download_root_path}/${new_tag}"
  else
    for item in "${FILE_LIST_PARTNAME[@]}";do
      file_name=${item}-${new_tag}.tar.gz
      file_url=${BASE_URL}/${file_name}
...
function resource::check_files_after_import(){
  ...
  wget https://github.com/kubean-io/kubean/releases/download/v0.5.4/files-amd64.list
  wget -q -c -T 1m -P ${download_root_path}/${file_name} "${download_root_path}/${new_tag}"
}
...
function resource::import_iso_local_path(){
